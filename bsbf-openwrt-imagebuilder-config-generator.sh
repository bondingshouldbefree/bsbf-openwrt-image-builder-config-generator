...
# be used with OpenWrt's imagebuilder to build an image with FusionLink.
...
$(curl -s $BSBF_FEED/bsbf-usb-netdev-autodhcp/files/etc/hotplug.d/net/99-bsbf-usb-netdev-autodhcp)
EOF
"
...
$(curl -s $BSBF_FEED/bsbf-quectel-usbnet/files/etc/init.d/bsbf-quectel-usbnet)
EOF
chmod +x /etc/init.d/bsbf-quectel-usbnet

cat <<'EOF' > /usr/sbin/bsbf-quectel-usbnet
$(curl -s $BSBF_FEED/bsbf-quectel-usbnet/files/usr/sbin/bsbf-quectel-usbnet)
EOF
...
$(curl -s $BSBF_FEED/bsbf-netspeed/files/usr/sbin/bsbf-netspeed)
EOF
...
# forwarding so that the device itself has access to a DNS server. Use 8.8.8.8
# because some ISPs such as rain SA won't reach 1.1.1.1. The vmess protocol
...
$(curl -s $BSBF_FEED/bsbf-tcp-in-udp/files/etc/hotplug.d/iface/99-bsbf-tcp-in-udp)
EOF

mkdir -p /usr/local/share/tcp-in-udp
cat <<'EOF' | base64 -d > /usr/local/share/tcp-in-udp/tcp_in_udp_tc.o
$(curl -s $BSBF_FEED/bsbf-tcp-in-udp/files/usr/local/share/tcp-in-udp/$tcp_in_udp_endianness | base64)
EOF
...
$(curl -s $BSBF_FEED/bsbf-mptcp-helper/files/etc/config/bsbf-mptcp-helper)
EOF

cat <<'EOF' > /etc/hotplug.d/iface/99-bsbf-mptcp-backup
$(curl -s $BSBF_FEED/bsbf-mptcp-helper/files/etc/hotplug.d/iface/99-bsbf-mptcp-backup)
EOF

cat <<'EOF' > /etc/hotplug.d/iface/99-bsbf-mptcp-remove
$(curl -s $BSBF_FEED/bsbf-mptcp-helper/files/etc/hotplug.d/iface/99-bsbf-mptcp-remove)
EOF

cat <<'EOF' > /etc/init.d/bsbf-mptcp-backup
$(curl -s $BSBF_FEED/bsbf-mptcp-helper/files/etc/init.d/bsbf-mptcp-backup)
EOF
chmod +x /etc/init.d/bsbf-mptcp-backup

cat <<'EOF' > /usr/sbin/bsbf-mptcp-backup
$(curl -s $BSBF_FEED/bsbf-mptcp-helper/files/usr/sbin/bsbf-mptcp-backup)
EOF
chmod +x /usr/sbin/bsbf-mptcp-backup

cat <<'EOF' > /usr/sbin/bsbf-mptcp-helper
$(curl -s $BSBF_FEED/bsbf-mptcp-helper/files/usr/sbin/bsbf-mptcp-helper)
EOF
...
$(curl -s $BSBF_FEED/bsbf-route/files/etc/init.d/bsbf-route)
EOF
chmod +x /etc/init.d/bsbf-route

cat <<'EOF' > /usr/sbin/bsbf-route
$(curl -s $BSBF_FEED/bsbf-route/files/usr/sbin/bsbf-route)
EOF
